...
async def init_db():
    ...
    db_exists = exists(sqlite_db_path)
    if not db_exists:
    ...
    async with get_new_db_connection() as conn:
        ...
        # Check if the database is properly initialized by checking for a core table
        # instead of just checking if the file exists
        if db_exists and await check_table_exists(organizations_table_name, cursor):
